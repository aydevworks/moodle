...
use core_course\external\course_without_summary_exporter;
...
class core_course_external extends external_api {
     ...
     * @return external_function_parameters
     */
    public static function get_enrolled_courses_without_summary_by_timeline_classification_parameters() {
        return new external_function_parameters(
            array(
                'classification' => new external_value(PARAM_ALPHA, 'future, inprogress, or past'),
                'limit' => new external_value(PARAM_INT, 'Result set limit', VALUE_DEFAULT, 0),
     ...
     * @throws  invalid_parameter_exception
     */
    public static function get_enrolled_courses_without_summary_by_timeline_classification(
        string $classification,
        int $limit = 0,
        int $offset = 0,
        string $sort = null,
        ...
        $requiredproperties = course_without_summary_exporter::define_properties();
        $fields = join(',', array_keys($requiredproperties));
        $hiddencourses = get_hidden_courses_on_timeline();
        $courses = [];
        ...
        $formattedcourses = array_map(function($course) use ($renderer, $favouritecourseids) {
            ...
            $exporter = new course_without_summary_exporter($course, ['context' => $context, 'isfavourite' => $isfavourite]);
            return $exporter->export($renderer);
        }, $filteredcourses);
     ...
     * @return \core_external\external_description
     */
    public static function get_enrolled_courses_without_summary_by_timeline_classification_returns() {
        return new external_single_structure(
            array(
                'courses' => new external_multiple_structure(course_without_summary_exporter::get_read_structure(), 'Course'),
                'nextoffset' => new external_value(PARAM_INT, 'Offset for the next request')
            )
        );
    }
