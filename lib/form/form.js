...
if (typeof M.form.dependencyManager === 'undefined') {
    ...
    Y.extend(dependencyManager, Y.Base, {
        ...
        _dependencyEq: function(elements, value, isHide) {
            ...
            elements.each(function() {
                if (this.getAttribute('type').toLowerCase() == 'radio') {
                    const formNode = this.get('form').getDOMNode();
                    const smartValue = formNode.elements.namedItem(this.get('name')).value;
                    hiddenVal = smartValue == value;
                    lock = lock || hiddenVal;
                    return;
